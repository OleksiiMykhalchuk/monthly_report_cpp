...
public:
	...
	void report(int m) {
		...
		{
			if (find(names.begin(), names.end(), row[i][0]) == names.end() && row[i][6].substr(7, 2) == to_string(m))
			{
				names.push_back(row[i][0]);
				rows.push_back(i);
			}
			month_number = stoi(row[i][6].substr(7, 2));
		...
		for (int i = 0;i < names_size;i++)
		{
			...
			{
				if (row[n][0] == row[j][0] && row[n][6] == row[j][6]) {
					
					sum = sum + stoi(row[j][7]);

				}
			}

			
			string year = row[1][6].substr(0, 4);
	...
	string not_excisted_message() {
		string message = "File is not excisted at the directory\nPlease check or create file.csv\n";
		return message;
	}
	void add_record() {
...
int main()
{
	...
	if (myreport.check_if_existed()) {
		...
		else if (answer != "y" || answer != "n") 
		{
			...
			{
				cout << endl;
				cout << "Entering record abborted!!!" << endl;
				cout << endl;
			}	
		}
		else 
	...
	{
		cout << myreport.not_excisted_message();
	}
}
